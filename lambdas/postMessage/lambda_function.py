...
def validate_event(event):
    ...
    if len(username) > 255:
        raise Exception("Bad Request: event['body-json']['username'] can't be larger than 255 characters, but is " + username)
    ...
    if type(message) is not str:
        logger.info(event)
        raise Exception("Bad Request: event['body-json']['message'] is not a str")
...
def lambda_handler(event, context):
    ...
    with conn.cursor() as cur:
        sql = "CALL insert_message(%s, %s)"
        result_code = cur.execute(sql, (parameters["username"], parameters["message"]))
        result = cur.fetchall()
