...
This is done outside the lambda handler because we don't need to 
initialize the connection every time we run the lambda
"""
try:
...
def validate_event(event):
    ...
    if type(querystring) is not dict:
        logger.info(event)
        raise Exception(
            "Bad Request: event['params']['querystring'] is not a dict")
    ...
    if "limit" in querystring:
        try:
            ...
            if limit < 0:
                logger.info(event)
                raise Exception(
                    "Bad Request: event['params']['querystring']['limit'] can't be negative but is " + querystring["limit"])
            if limit > max_limit:
                logger.info(event)
            ...
            raise Exception(
                "Bad Request: event['params']['querystring']['limit'] '" + querystring["limit"] + "' is not a valid number")
    else:
        limit = max_limit
...
def handler(event, context):
    ...
    """
    limit = validate_event(event)
    with connection.cursor() as cursor:
        connection.commit()
        statement = "SELECT id, username, message, created_time FROM messages ORDER BY created_time DESC LIMIT %s"
